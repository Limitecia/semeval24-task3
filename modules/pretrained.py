...
class PretrainedEmbedding(nn.Module):
    def __init__(self, pretrained: str, embed_size: int, pad_index: int, finetune: bool, device: str):
        super().__init__()
        self.word_embed = AutoModel.from_pretrained(pretrained, device_map='auto').requires_grad_(finetune)
    ...
    def forward(self, words: torch.Tensor) -> torch.Tensor:
        ...
        word_lens = word_lens[word_lens > 0]
        x = split([torch.mean(i, dim=0) for i in x[flat != self.pad_index].split(word_lens.tolist())], [l for l in lens if l > 0])
